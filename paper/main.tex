...
In controlled synthetic benchmarks with one warm-up and five measured repetitions, the Rust layout backend processed one-million-segment chain, branched and fragmented graphs in 5.69, 13.29 and 7.02~s, respectively. The optional CUDA backend reduced the branched case to 10.12~s but was slower on chain and fragmented graphs. \bandageng exceeded the 900~s timeout on all three one-million-segment cases, while \bandage returned errors for all measured 500,000- and one-million-segment runs in the present benchmark environment. On an HPRC chromosome 22 graph, \graphite completed in 27.67~s with Rust layout and 18.39~s with CUDA, whereas both external viewers exceeded 900~s.
\end{abstract}
...
\midrule
Chain & \graphite{} Rust & 5.69 & 1.78\\
      & \graphite{} CUDA & 6.41 & 1.83\\
      & \graphite{} OGDF & 5.65 & 1.78\\
      & \bandageng & TO & --\\
      & \bandage & ERR & --\\
\addlinespace
Branched & \graphite{} Rust & 13.29 & 1.80\\
         & \graphite{} CUDA & 10.12 & 1.84\\
         & \graphite{} OGDF & 90.59 & 5.35\\
         & \bandageng & TO & --\\
         & \bandage & ERR & --\\
\addlinespace
Fragmented & \graphite{} Rust & 7.02 & 1.81\\
           & \graphite{} CUDA & 7.68 & 1.85\\
           & \graphite{} OGDF & 7.17 & 1.81\\
           & \bandageng & TO & --\\
           & \bandage & ERR & --\\
\bottomrule
\end{tabular}
...
On the 500,000-segment branched graph, \graphite Rust completed in 6.83~s with 0.94~GiB peak RSS, while CUDA took 5.61~s with 1.01~GiB; \bandageng took 451.62~s and 10.39~GiB. On the 100,000-segment branched graph, \graphite Rust required 1.38~s and 221~MiB, while CUDA required 1.58~s and 331~MiB. \bandageng required 70.10~s and 2.25~GiB and \bandage required 67.91~s and 1.72~GiB.

For fragmented graphs, the one-million-segment Rust run completed in 7.02~s with 1.81~GiB peak RSS; CUDA took 7.68~s with 1.85~GiB. At 500,000 segments Rust completed in 3.41~s with 924~MiB, while \bandageng timed out and \bandage failed. The Graphite OGDF backend had nearly identical end-to-end times for the chain and fragmented cases, while Rust was 6.82 times faster than OGDF on the large branched graph.
...
\midrule
Branched 10k & 0.510 & 0.867 & 2.447 & 5.387 & 6.353\\
Branched 100k & 1.377 & 1.582 & 7.995 & 70.103 & 67.913\\
Branched 500k & 6.83 & 5.61 & 42.30 & 451.62 & ERR\\
Branched 1M & 13.29 & 10.12 & 90.59 & TO & ERR\\
\addlinespace
Fragmented 10k & 0.103 & 0.511 & 0.103 & 1.528 & 4.980\\
Fragmented 100k & 0.612 & 1.122 & 0.664 & 19.160 & 46.682\\
Fragmented 500k & 3.41 & 3.87 & 3.31 & TO & ERR\\
Fragmented 1M & 7.02 & 7.68 & 7.17 & TO & ERR\\
\bottomrule
\end{tabular}
\end{table}

\subsection{Real assembly graphs}
On the SPAdes ESC graph, \graphite Rust completed in 0.205~s with 19.2~MiB peak RSS, compared with 1.12~s and 55.8~MiB for \bandageng and 1.78~s and 67.4~MiB for \bandage. On the MEGAHIT 5G graph, the corresponding values were 0.510~s and 56.4~MiB for \graphite Rust, 3.82~s and 360~MiB for \bandageng, and 6.15~s and 299~MiB for \bandage.

The largest real graph produced the clearest separation. On HPRC chromosome 22, \graphite Rust completed in 27.67~s with 3.81~GiB peak RSS. CUDA completed in 18.39~s with 3.90~GiB, and the optional OGDF reference backend completed in 177.69~s with 10.00~GiB. Standalone \bandage and \bandageng both exceeded 900~s on all five measured runs. Figures~\ref{fig:real-wall} and~\ref{fig:real-rss} compare the real-graph wall times and peak memory; Tables~\ref{tab:real-wall} and~\ref{tab:real-rss} give the corresponding values.
...
Dataset & Graphite Rust & Graphite CUDA & Graphite OGDF & BandageNG & Bandage\\
\midrule
BRCA2 & 0.205 & 0.562 & 0.357 & 0.764 & 1.069\\
SPAdes ESC & 0.205 & 0.613 & 0.662 & 1.120 & 1.781\\
Flye & 0.103 & 0.511 & 0.103 & 0.510 & 0.764\\
MEGAHIT 5G & 0.510 & 1.272 & 1.527 & 3.822 & 6.149\\
myloasm & 0.052 & 0.459 & 0.052 & 0.204 & 0.255\\
hifiasm chr11 & 0.052 & 0.459 & 0.052 & 0.255 & 0.357\\
HPRC chr22 & 27.67 & 18.39 & 177.69 & TO & TO\\
\bottomrule
\end{tabular}
\end{table}
...
Dataset & Graphite Rust & Graphite CUDA & Graphite OGDF & BandageNG & Bandage\\
\midrule
BRCA2 & 11.3 & 162.1 & 18.1 & 53.4 & 41.8\\
SPAdes ESC & 19.2 & 172.5 & 25.6 & 55.8 & 67.4\\
Flye & 30.5 & 169.9 & 30.5 & 42.2 & 79.6\\
MEGAHIT 5G & 56.4 & 205.6 & 70.2 & 360.2 & 299.0\\
myloasm & 8.8 & 147.7 & 8.7 & 29.0 & 21.9\\
hifiasm chr11 & 10.2 & 149.0 & 10.2 & 32.1 & 36.5\\
HPRC chr22 & 3903 & 3990 & 10240 & TO & TO\\
\bottomrule
\end{tabular}
\end{table}

\subsection{CPU utilization and total CPU work}
CPU accounting shows that the wall-time differences cannot be explained simply by Graphite consuming more cores. Table~\ref{tab:cpu} summarizes representative CPU utilization for CPU-only backends; CUDA device work is outside this accounting. On the 500,000-segment branched graph, \graphite Rust used a median 22.47 CPU-s over 6.83~s wall time, with 3.22 median average cores per run. The optional OGDF reference backend used 42.21 CPU-s over 42.30~s (approximately one core), while \bandageng used 451.47 CPU-s over 451.62~s (also approximately one core). Thus the Rust backend was 66.2-fold faster in wall time than \bandageng while consuming about 20-fold less total CPU time on this workload.

The difference is even more pronounced for fragmented graphs, where \bandageng uses substantially more parallelism. On the 100,000-segment fragmented graph, \graphite Rust used 0.56 CPU-s over 0.612~s, or 0.93 median average cores per run, whereas \bandageng used 238.03 CPU-s over 19.16~s, or 12.43 average cores. \graphite completed 31.3-fold sooner while consuming much less total CPU time. On the MEGAHIT 5G graph, \graphite Rust used 1.62 CPU-s over 0.510~s (3.18 average cores), compared with 45.18 CPU-s over 3.82~s (12.12 average cores) for \bandageng.

On HPRC chromosome 22, \graphite Rust consumed 100.93 CPU-s over 27.67~s, corresponding to 3.63 median average cores per run. The OGDF reference backend consumed 177.55 CPU-s over 177.69~s, or approximately one core. CPU totals are unavailable for the standalone \bandage and \bandageng HPRC runs because they reached the 900~s timeout.

These results also separate the effects of parallelism from algorithmic work. On the one-million-segment branched graph, Rust used less than half the total CPU time of OGDF (43.72 versus 90.50 CPU-s), while parallel execution further reduced wall time from 90.59 to 13.29~s. On the one-million-segment fragmented graph, both backends used approximately 7.0 CPU-s and 7.0~s wall time, so the end-to-end comparison shows little backend difference for this topology.
...
\midrule
Branched 500k & \graphite{} Rust & 6.83 & 22.47 & 3.22\\
               & \graphite{} OGDF & 42.30 & 42.21 & 1.00\\
               & \bandageng & 451.62 & 451.47 & 1.00\\
\addlinespace
Fragmented 100k & \graphite{} Rust & 0.612 & 0.56 & 0.93\\
                 & \graphite{} OGDF & 0.664 & 0.61 & 0.92\\
                 & \bandageng & 19.16 & 238.03 & 12.43\\
                 & \bandage & 46.68 & 46.43 & 0.99\\
\addlinespace
MEGAHIT 5G & \graphite{} Rust & 0.510 & 1.62 & 3.18\\
            & \graphite{} OGDF & 1.53 & 1.49 & 0.97\\
            & \bandageng & 3.82 & 45.18 & 12.12\\
            & \bandage & 6.15 & 5.91 & 0.96\\
\addlinespace
HPRC chr22 & \graphite{} Rust & 27.67 & 100.93 & 3.63\\
            & \graphite{} OGDF & 177.69 & 177.55 & 1.00\\
\bottomrule
\end{tabular}
\end{table}

\subsection{Runtime composition}
Internal timings show that layout remains the largest stage in the largest connected cases. For HPRC chromosome 22, median internal timings were 4.47~s for parsing, 0.77~s for view-graph construction, 18.32~s for initial Rust layout and 3.82~s for SVG export. CUDA reduced initial layout to 8.61~s; its parsing and export times were 4.55 and 3.76~s. On the one-million-segment branched synthetic graph, Rust parsing required 1.10~s and initial layout 8.61~s, compared with 5.35~s for CUDA layout. On the one-million-segment fragmented graph, Rust parsing and layout took 0.91 and 2.66~s; SVG export took another 2.58~s. CUDA layout took 3.15~s for that graph.

\subsection{Canvas responsiveness}
Table~\ref{tab:canvas-work} reports the CPU-side canvas-work measurements for the Graphite layout. For the million-segment branched graph, the median fitted-overview frame took 1.44~ms and the median 1:1 detail frame took 9.84~ms. The corresponding median 95th-percentile frame times across the five measured processes were 1.49 and 10.28~ms. Building the settled-layout cache took a median 67.01~ms once per measured process, outside the frame timings. The fitted overview produced a median 5,533 shapes per frame, compared with 1,283 at the measured detail view. These CPU-side observations do not establish the application's full presented frame rate or input-to-display latency.
...
\midrule
100,000 & 0.98 & 1.02 & 0.93 & 1.04 & 10.09\\
1,000,000 & 1.44 & 1.49 & 9.84 & 10.28 & 67.01\\
\bottomrule
\end{tabular}
\end{table}

\section{Discussion}
The results indicate that both compact input representation and the Rust-native layout contribute to the scale at which complete assembly graphs can be explored. The optional OGDF backend serves as a validation and reference standard. Because it runs inside the same Graphite parser, graph model and export pipeline, it provides a controlled within-application comparison: OGDF takes approximately 6.82 times as long on the one-million-segment branched graph and 6.42 times as long on HPRC chromosome 22. Chain and fragmented synthetic cases have nearly equal end-to-end times in Graphite. CUDA reduced HPRC end-to-end time by about one third and the branched million-segment case by about one quarter relative to Rust, but its setup and data transfers made it slower on smaller graphs and the chain and fragmented million-segment cases. These measurements characterize the NVIDIA CUDA implementation; the portable Vulkan, Metal and Direct3D backend was not included in the publication run.
