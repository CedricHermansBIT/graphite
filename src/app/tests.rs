use super::*;
use crate::app_core::InteractionMode;
...
fn session_round_trip_restores_geometry_view_selection_and_rejects_changed_source() {
    ...
    app.core.display.theme = ThemePreset::Paper;
    app.core.zoom = 2.5;
    app.core.pan = Vec2::new(31.0, -14.0);
    app.core.selection.nodes.insert(0);
    app.core.overlays.selected_path = Some(0);
    let session = directory.path().join("view.graphite.json");
    app.start_output(session.clone(), OutputKind::Session);
    settle(&mut app, &context);
    assert!(session.is_file(), "{}", app.core.status_msg);
    drop(app);
    ...
    let LoadState::Loaded {
        ...
        panic!("{}", restored.core.status_msg)
    };
    assert_eq!(layout_snapshot.positions, positions);
    assert_eq!(restored.core.display.theme, ThemePreset::Paper);
    assert_eq!(restored.core.zoom, 2.5);
    assert_eq!(restored.core.pan, Vec2::new(31.0, -14.0));
    assert!(restored.core.selection.nodes.contains(&0));
    assert_eq!(restored.core.overlays.selected_path, Some(0));
    assert!(!restored.core.pending_fit);
    drop(restored);
    ...
    assert!(
        rejected.core.status_msg.contains("changed since"),
        "{}",
        rejected.core.status_msg
    );
}
...
fn cancel_or_failed_load_retains_previous_graph() {
    ...
    settle(&mut app, &context);
    assert!(matches!(app.load_state, LoadState::Loaded { .. }));
    assert!(app.core.status_msg.contains("failed"));
}
...
fn warnings_are_visible_and_strict_mode_rejects_them() {
    ...
    settle(&mut app, &context);
    assert!(app.core.show_diagnostics);
    assert!(app.core.status_msg.contains("1 warnings"));
    let mut strict = empty_app(&context);
    strict.core.strict_parsing = true;
    strict.start_load(path);
    settle(&mut strict, &context);
    assert!(!matches!(strict.load_state, LoadState::Loaded { .. }));
    assert!(strict.core.status_msg.contains("Strict parsing rejected"));
}
...
fn typing_into_search_does_not_change_mode_or_fit_view() {
    ...
    let context = Context::default();
    let mut app = loaded_app(&context, directory.path());
    app.core.pending_fit = false;
    app.core.interaction_mode = InteractionMode::Pan;
    app.core.zoom = 1.75;
    ...
    for key in [Key::G, Key::F, Key::S, Key::P] {
        ...
        assert!(app.core.interaction_mode == InteractionMode::Pan);
        assert_eq!(app.core.zoom, 1.75);
    }
}
...
fn appearance_and_recent_files_persist_without_sequences() {
    ...
    let context = Context::default();
    let mut app = empty_app(&context);
    app.core.display.theme = ThemePreset::Light;
    app.core.display.show_labels = false;
    ...
    assert_eq!(restored.core.display.theme, ThemePreset::Light);
    assert!(!restored.core.display.show_labels);
    assert_eq!(
...
fn movement_undo_redo_restores_exact_positions() {
    ...
    let context = Context::default();
    let mut app = loaded_app(&context, directory.path());
    app.core.grabbed_phys = Some(0);
    {
        let (core, load_state) = (&mut app.core, &app.load_state);
        if let LoadState::Loaded {
            view,
            layout_snapshot,
            ..
        ...
        {
            core.begin_edit(layout_snapshot, view, 0);
        } else {
            unreachable!();
        }
    }
    let pending = app.core.pending_edit.as_ref().unwrap();
